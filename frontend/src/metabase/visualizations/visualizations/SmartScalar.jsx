    ...
    if (!insight) {
      ...
      if (bucketIdx > -1) {
        const valueIdx = _.findIndex(
          cols,
          val => val.display_name === column.name,
        );
      ...
      <ScalarWrapper>
        ...
        <div className="SmartWrapper">
          {!trends.length ? (
            ...
            >{jt`Nothing to compare for the previous .`}</div>
          ) : (
            <div>
              {trends.map((trend, idx) => {
                return (
                  <PreviousValueContainer key={idx}>
                    <Variation color={trend.changeColor}>
                      <Icon
                        size={13}
                        pr={1}
